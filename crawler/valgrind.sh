...
valgrind --leak-check=full --show-leak-kinds=all crawler.bin https://thayer.github.io/engs50/ ../pages 2 2> valgrind_err_2 > valgrind_out_2
if [[ $(grep 'no leaks are possible' valgrind_err_2 | wc -l) -eq 1 && $(grep '0 errors' valgrind_err_2 | wc -l) -eq 1 ]]
then
		echo "crawler.bin passed valgrind check"
else
		echo "crawler.bin FAILED valgrind check"
		cat valgrind_err_2
fi
